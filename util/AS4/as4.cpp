// Intel 4004/4040 assembler compiler v1.0
// (c) 2020 Miroslav Nemecek, panda38.sweb.cz
...
Non-standard extensions:
	.strict		- strict mode:
					...
					- register pairs must have name R01..R1415
	...
	*=addr[,fill] - set address, can have fill byte or 0 default
	.set a b	- 'a' will by substituted by 'b' (text without spaces)
...
group 0x10..0xDF:
	...
	SUB r ... subtract register r and C from A (A = A + ~r + ~C, C is 1 if not borrow)
...
group 0xF0..0xFF:
	...
	CLC .. clear C
	IAC (INC A) ... increrment A (overflow -> C)
	CMC ... complement C
	CMA ... complement A
	RAL ... rotate A left through C (C<-a3<-a2<-a1<-a0<-C)
	RAR ... rotate A right through C (C->a3->a2->a1->a0->C)
	TCC ... transmit C to A (0 or 1), clear C
	DAC (DEC A) ... decrement A (~borrow -> C)
	TCS ... transfer C subtract to A (0->9 or 1->10), clear C
	STC ... set carry
	DAA ... decimal adjust A (add 6 if A>9 or C=1 and carry->C; C not affected otherwise)
	KBP ... keyboard process (A change "1 of 4" to bit number, or 15 if invalid)
	DCL ... designate command line (high byte of RAM/ROM address)
*/
...
int main(int argc, char **argv)
{
	...
	// print info
	printf("Intel 4004/4040 assembler compiler v1.0\n");
	printf("(c) 2020 Miroslav Nemecek, panda38.sweb.cz\n");

	// check syntax
	if (argc != 2)
	{
		printf("Syntax: as4 file.s\n");
		return 1;
	}
